...
This server uses **AuthKit OAuth2.0** (from WorkOS) for authentication. AuthKit provides enterprise-grade OAuth2.0 authentication with support for various identity providers.

### Configuration

To enable AuthKit authentication, set the following environment variables:
...
If these variables are not set, the server will run without authentication.

### Supported Environment Variables

- `AUTHKIT_DOMAIN`: Your AuthKit domain (e.g., `your-app.authkit.app`)
- `BASE_URL`: Your application's base URL
- `FASTMCP_SERVER_AUTH_AUTHKITPROVIDER_AUTHKIT_DOMAIN`: Alternative AuthKit domain variable
- `FASTMCP_SERVER_AUTH_AUTHKITPROVIDER_BASE_URL`: Alternative base URL variable
...
### 2. Authentication (AuthKit OAuth2.0)
- **Provider**: WorkOS AuthKit
- **Purpose**: OAuth2.0 authentication for all tools
- **Configuration**: Set `AUTHKIT_DOMAIN` and `BASE_URL` environment variables
